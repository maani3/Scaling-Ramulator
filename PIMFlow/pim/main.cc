...
std::vector<std::string> PimCodeGen(std::string id, const Str2StrMap& attrs,
  ...
  int h = std::stoi(attrs.at("H"));       //input_height
  int w = std::stoi(attrs.at("W"));       //input_width
  int kh = std::stoi(attrs.at("KH"));     //kernel height
  int kw = std::stoi(attrs.at("KW"));     //kernel width
  int ph = std::stoi(attrs.at("PH"));     //padding height
  int pw = std::stoi(attrs.at("PW"));     //padding width
  int stride_ = std::stoi(attrs.at("S")); //kernel stride

  int H = (h - kh + 2 * ph) / stride_ + 1; //output height
  int W = (w - kw + 2 * pw) / stride_ + 1; //output width
  int C_o = std::stoi(attrs.at("K"));      //output channel, which is the same as the number of kernels/filters in convolution
  int C_i = std::stoi(attrs.at("C"));      //input channel, which is the same as the number of channels in convolution

  auto i_c = (((C_i * kh * kw) + 15) / 16) * 16; //number of elements in flattened kernel
  auto o_c = C_o;                                //number of output channels
...
std::vector<std::string> PimCodeGenBGScaled(std::string id, const Str2StrMap& attrs,
                const std::vector<std::string>& func_args) {
  int n_channel = std::stoi(attrs.at("N_CHANNEL"));
  std::vector<std::string> code;
  std::ostringstream OS;
  ...
  auto i_c = (((C_i * kh * kw) + 15) / 16) * 16; //number of elements in flattened kernel
  auto o_c = C_o;                                //number of output channels
  auto row = ((o_c + 31) / 32) * 32;             //32 banks per channel now
  int fl, stride, n, col;

  // Boundary condition is ignored (e.g., 3x3 kernel with (1, 1) padding)
  if (i_c <= 512) {
    fl = 512 / i_c;
    col = i_c * fl;
    stride = i_c;
    ...
    for (int i = 0; i < n; i++) {
      ...
      pim::OutputNewtonTraceV2BG(OS, id, row, col, stride, sinfo);
    }
    code.push_back(OS.str());
  } else {
    int full = i_c / 512;
    ...
    for (int j = 0; j < full; j++) {
      for (int i = 0; i < n; i++) {
        pim::OutputNewtonTraceV2BG(OS, id, row, col, stride);
      }
    }
    code.push_back(OS.str());
    OS.str("");
...
std::vector<std::string> PimCodeGenFullScaled(std::string id, const Str2StrMap& attrs,
                const std::vector<std::string>& func_args) {
  int n_channel = std::stoi(attrs.at("N_CHANNEL"));
  std::vector<std::string> code;
  std::ostringstream OS;
  ...
  if (i_c <= 1024) {
    ...
    for (int i = 0; i < n; i++) {
      ...
      if (kh > 1 || kw > 1) {
        ...
        sinfo.stride = C_i * (h - kh);
        sinfo.num_after_elem = C_i * kh;
        sinfo.num_gwrite = 1024 / (C_i * kh * kw);
      }
      pim::OutputNewtonTraceV2scaled(OS, id, row, col, stride, sinfo);
    }
    code.push_back(OS.str());
  } else {
    int full = i_c / 1024;
    ...
    for (int j = 0; j < full; j++) {
      for (int i = 0; i < n; i++) {
        pim::OutputNewtonTraceV2scaled(OS, id, row, col, stride);
      }
    }
    code.push_back(OS.str());
    OS.str("");
    OS.clear();
    int i_c_remain = i_c - (1024 * full);
    if (i_c_remain > 0) {
      // i_c_remain = nextPow2(i_c_remain);
      i_c_remain = ((i_c_remain + 15) / 16) * 16;
      fl = 1024 / i_c_remain;
...
void get_args(int argc, char *argv[], Str2StrMap& attrs) {
  ...
  while (pos < argc) {
    ...
    if (pos + 1 < argc && !strcmp(argv[pos], "-codegen")) {
      attrs["CODEGEN"] = argv[pos + 1];
      pos += 2;
      continue;
    }
    pos += 1;
...
int main(int argc, char *argv[]) {
  ...
  std::string mode = attrs["CODEGEN"];

  std::vector<std::string> code;

  if (mode == "8bg") {
      code = PimCodeGenBGScaled(kernel_name, attrs, {});
  } 
  else if (mode == "full") {
      code = PimCodeGenFullScaled(kernel_name, attrs, {});
  } 
  else {
      code = PimCodeGen(kernel_name, attrs, {});
  }
