...
void GWrite(std::ostream& OS, char* buf, int r, StrideInfo sinfo) {
  //Changed first %02d to %03d, for 8 BG --mahasin !!
  sprintf(buf, "%03d%02d%s%05d%06d", 0, 0, FillZero(ToBinary(r), 14).c_str(), 0, 0);
...
void GAct(std::ostream& OS, char* buf, int k, int r, int j, int num_act) {
  for (int i = 0; i < num_act; i++) {
    //Changed first FillZero arg to 3, for 8 BG --mahasin !!
    sprintf(buf, "%s%02d%s%05d%06d", FillZero(ToBinary(i), 3).c_str(), 0, FillZero(ToBinary(k*r+j), 14).c_str(), 0, 0);
...
void Comp(std::ostream& OS, char* buf, int k, int r, int j, int h) {
  //Changed first %02d to %03d, for 8 BG --mahasin !!
  sprintf(buf, "%03d%02d%s%s%06d", 0, 0, FillZero(ToBinary(k*r+j), 14).c_str(), FillZero(ToBinary(h), 5).c_str(), 0);
...
void OutputNewtonTraceV2(std::ostream& OS, std::string kernel_name, int64_t row, int64_t col, int64_t stride, StrideInfo sinfo) {
  ...
  for (int i = 0; i < num_chunks; i++) {
    ...
    for (int g = 0; g < sinfo.num_gwrite; g++) {
      GWrite(OS, buf, i, sinfo); //might half!! -- mahasin
    ...
    for (int j = 0; j < r; j++) {
      ...
      //   num_act = ((row - (row/16)*16) + 3) / 4;
      // }
      GAct(OS, buf, i, r, j, num_act); //GACT should go down!
      int bound = 32; //COMP should double -- mahasin!!
      if (col % 512 != 0) {
        bound = ((col - (col/512)*512) + 15) / 16;
      }
      for (int k = 0; k < bound; k++) {
...
void OutputNewtonTraceV2BG(std::ostream& OS, std::string kernel_name, int64_t row, int64_t col, int64_t stride, StrideInfo sinfo) {
  char buf[100]; // fixed size
...
void OutputNewtonTraceV2scaled(std::ostream& OS, std::string kernel_name, int64_t row, int64_t col, int64_t stride, StrideInfo sinfo) {
  char buf[100]; // fixed size

  int num_chunks = (col + 1023) / 1024;
  int r = (row + 31) / 32;
  ...
  for (int i = 0; i < num_chunks; i++) {
    ...
    for (int j = 0; j < r; j++) {
      ...
      // }
      GAct(OS, buf, i, r, j, num_act); //GACT should go down!
      int bound = 64; //COMP should double -- mahasin!!
      if (col % 1024 != 0) {
        bound = ((col - (col/1024)*1024) + 15) / 16;
      }
      for (int k = 0; k < bound; k++) {
        Comp(OS, buf, i, r, j, k);
        elem += 16;
